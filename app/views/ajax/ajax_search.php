  ...
  <div id="content">
    <form id="searchForm" action="comment.php" method="post">    
      <div id="searchBar">
        ...
        <div id="searchResult">
          <div style="padding: 3px 0;">Aranacak firmayı girin</div>
        </div>
        <script type="text/javascript">
          ...
          searchInput.onkeyup = function () {
            if (this.value.length > 0) {
              ...
              for (var i = 0; i < finds.length; i++) {
                if (i % 2 == 1)
                  result += '<a onclick="" href="comment.php?firm_id='+finds[i].id+'"><div class="searchItem odd">'+finds[i].name+'</div></a>';
                else
                  result += '<a onclick="" href="comment.php?firm_id='+finds[i].id+'"><div class="searchItem">'+finds[i].name+'</div></a>';
              }
              if (result.length == 0) {
          ...
          window.onload = function () {
            ...
            var firmList = '';
            for (var i = 0; i < firms.length; i++) {
              firmList += '<li><div class="firmLink"><a href="comment.php?firm_id=' + firms[i].id + '">' + firms[i].name + '</a></div>'
